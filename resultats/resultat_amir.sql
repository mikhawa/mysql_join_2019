...
SELECT theuser.thelogin, theuser.thename, perm.thename as permname FROM theuser JOIN perm ON theuser.perm_idperm = perm_idperm; 
...
SELECT theuser.thelogin, theuser.thename, perm.thename as permname FROM theuser LEFT JOIN perm ON theuser.perm_idperm = perm.idperm WHERE perm.thelevel = 3;
...
SELECT theuser.idtheuser, theuser.thelogin, theuser.thename, perm.thename as permname, article.thetitle FROM theuser LEFT JOIN article ON article.theuser_idtheuser = theuser.idtheuser JOIN perm ON perm.idperm = theuser.perm_idperm;
...
SELECT theuser.idtheuser, theuser.thelogin, theuser.thename, perm.thename AS permname, article.thetitle FROM theuser JOIN article ON article.theuser_idtheuser = theuser.idtheuser JOIN perm ON perm.idperm = theuser.perm_idperm;
...
SELECT DISTINCT theuser.thelogin, theuser.thename, theuser.idtheuser,  perm.thename AS permname, GROUP_CONCAT(article.idarticle) AS idarticle, GROUP_CONCAT(article.thetitle SEPARATOR "|||") AS thetitle FROM theuser JOIN article ON article.theuser_idtheuser = theuser.idtheuser JOIN perm ON perm.idperm = theuser.perm_idperm GROUP BY theuser.idtheuser;
...
SELECT theuser.idtheuser, theuser.thelogin, theuser.thename, perm.thename AS permname, GROUP_CONCAT(article.idarticle) AS idarticle, GROUP_CONCAT(article.thetitle SEPARATOR "|||") AS thetitle FROM theuser LEFT JOIN article ON article.theuser_idtheuser = theuser.idtheuser  JOIN perm ON perm.idperm = theuser.perm_idperm GROUP BY theuser.thelogin DESC;
...
SELECT article.idarticle, article.thetitle, article.thedate, image.theurl, image.thealt FROM article LEFT JOIN image ON image.article_idarticle = article.idarticle;  
...
SELECT DISTINCT article.thetitle, article.idarticle, article.thedate, GROUP_CONCAT(image.theurl SEPARATOR "|||") AS theurl, GROUP_CONCAT(image.thealt SEPARATOR "|$|") AS thealt FROM article LEFT JOIN image ON image.article_idarticle = article.idarticle GROUP BY article.thedate DESC; 
...
SELECT article.idarticle, article.thetitle, article.thedate,  GROUP_CONCAT(categ.idcateg) AS idcateg, GROUP_CONCAT(categ.thetitle SEPARATOR "_o!o_") ctitle FROM article LEFT JOIN categ_has_article ON categ_has_article.article_idarticle = article.idarticle LEFT JOIN categ ON categ.idcateg = categ_has_article.categ_idcateg GROUP BY article.idarticle DESC; 
...
SELECT article.idarticle, article.thetitle, article.thedate, GROUP_CONCAT(categ.idcateg) AS idcateg, GROUP_CONCAT(categ.thetitle SEPARATOR "_o!o_") AS ctitle, theuser.idtheuser, theuser.thelogin FROM article LEFT JOIN categ_has_article ON categ_has_article.article_idarticle = article.idarticle LEFT JOIN categ ON categ.idcateg = categ_has_article.categ_idcateg JOIN theuser ON theuser.idtheuser = article.theuser_idtheuser GROUP BY article.thedate DESC;